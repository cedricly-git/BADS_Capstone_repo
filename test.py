...
# ============== TEST SCENARIOS ==============
TEST_SCENARIOS = {
    "Live Data (Real Predictions)": None,  # Use actual ML predictions
    "High Demand Week": {
        "demand_multipliers": [1.25, 1.35, 1.20, 1.40, 1.30, 1.45, 1.22],
        "weather": "mixed"
    },
    "Low Demand Week": {
        "demand_multipliers": [0.75, 0.70, 0.82, 0.68, 0.78, 0.72, 0.80],
        "weather": "mixed"
    },
    "Very High Demand (Critical)": {
        "demand_multipliers": [1.50, 1.55, 1.48, 1.60, 1.52, 1.58, 1.45],
        "weather": "mixed"
    },
    "Mixed Week (Typical)": {
        "demand_multipliers": [0.95, 1.10, 0.88, 1.05, 1.15, 1.25, 0.92],
        "weather": "mixed"
    },
    "Rainy Week": {
        "demand_multipliers": [1.05, 1.08, 1.02, 1.10, 1.06, 1.12, 1.04],
        "weather": "rainy"
    },
    "Sunny & Warm Week": {
        "demand_multipliers": [0.98, 1.02, 0.95, 1.00, 1.05, 1.08, 0.96],
        "weather": "sunny_warm"
    },
    "Cold Winter Week": {
        "demand_multipliers": [1.08, 1.12, 1.05, 1.15, 1.10, 1.18, 1.06],
        "weather": "cold"
    },
    "Weekend Rush": {
        "demand_multipliers": [0.85, 0.80, 0.90, 0.88, 1.35, 1.45, 0.95],
        "weather": "mixed"
    },
    "Weekday Peak": {
        "demand_multipliers": [1.30, 1.25, 1.35, 1.28, 0.90, 0.85, 1.20],
        "weather": "mixed"
    },
    "Single Peak Day (Friday)": {
        "demand_multipliers": [0.90, 0.88, 0.92, 0.95, 1.55, 0.98, 0.85],
        "weather": "mixed"
    },
}

# Weather presets for scenarios
WEATHER_PRESETS = {
    "mixed": [
        {"Temp_Max": 18, "Temp_Min": 10, "Precipitation": 0.5},
        {"Temp_Max": 15, "Temp_Min": 8, "Precipitation": 5.2},
        {"Temp_Max": 20, "Temp_Min": 12, "Precipitation": 0.0},
        {"Temp_Max": 16, "Temp_Min": 9, "Precipitation": 2.1},
        {"Temp_Max": 22, "Temp_Min": 14, "Precipitation": 0.0},
        {"Temp_Max": 19, "Temp_Min": 11, "Precipitation": 1.5},
        {"Temp_Max": 17, "Temp_Min": 10, "Precipitation": 3.0},
    ],
    "rainy": [
        {"Temp_Max": 14, "Temp_Min": 8, "Precipitation": 8.5},
        {"Temp_Max": 12, "Temp_Min": 7, "Precipitation": 12.0},
        {"Temp_Max": 15, "Temp_Min": 9, "Precipitation": 6.2},
        {"Temp_Max": 13, "Temp_Min": 8, "Precipitation": 9.8},
        {"Temp_Max": 11, "Temp_Min": 6, "Precipitation": 15.0},
        {"Temp_Max": 14, "Temp_Min": 8, "Precipitation": 7.5},
        {"Temp_Max": 16, "Temp_Min": 10, "Precipitation": 4.2},
    ],
    "sunny_warm": [
        {"Temp_Max": 28, "Temp_Min": 18, "Precipitation": 0.0},
        {"Temp_Max": 30, "Temp_Min": 20, "Precipitation": 0.0},
        {"Temp_Max": 27, "Temp_Min": 17, "Precipitation": 0.0},
        {"Temp_Max": 29, "Temp_Min": 19, "Precipitation": 0.0},
        {"Temp_Max": 31, "Temp_Min": 21, "Precipitation": 0.0},
        {"Temp_Max": 28, "Temp_Min": 18, "Precipitation": 0.0},
        {"Temp_Max": 26, "Temp_Min": 16, "Precipitation": 0.2},
    ],
    "cold": [
        {"Temp_Max": 4, "Temp_Min": -2, "Precipitation": 1.5},
        {"Temp_Max": 2, "Temp_Min": -4, "Precipitation": 0.5},
        {"Temp_Max": 5, "Temp_Min": -1, "Precipitation": 2.0},
        {"Temp_Max": 3, "Temp_Min": -3, "Precipitation": 0.0},
        {"Temp_Max": 1, "Temp_Min": -5, "Precipitation": 3.0},
        {"Temp_Max": 4, "Temp_Min": -2, "Precipitation": 1.0},
        {"Temp_Max": 6, "Temp_Min": 0, "Precipitation": 0.5},
    ],
}
...
def generate_scenario_data(scenario_name, historical_stats):
    """Generate mock forecast data based on selected scenario"""
    scenario = TEST_SCENARIOS.get(scenario_name)
    if scenario is None:
        return None  # Use real data
    
    # Get base values
    base_demand = historical_stats['mean']
    weather_preset = WEATHER_PRESETS[scenario['weather']]
    demand_multipliers = scenario['demand_multipliers']
    
    # Generate 7 days of data starting from today
    today = datetime.now().date()
    data = []
    
    for i in range(7):
        day = today + timedelta(days=i)
        weather = weather_preset[i]
        predicted_searches = base_demand * demand_multipliers[i]
...
# Page configuration
st.set_page_config(
    page_title="Uber Eats Demand Forecast - TEST MODE",
    layout="wide",
...
if 'selected_scenario' not in st.session_state:
    st.session_state.selected_scenario = "Live Data (Real Predictions)"
...
def show_sidebar():
    with st.sidebar:
        ...
        st.markdown("*Select a scenario to preview*")
        
        scenario_options = list(TEST_SCENARIOS.keys())
        # Get current index from session state
        current_scenario = st.session_state.get('selected_scenario', 'Live Data (Real Predictions)')
        current_index = scenario_options.index(current_scenario) if current_scenario in scenario_options else 0
        ...
        current_scenario_display = st.session_state.selected_scenario
        if current_scenario_display != "Live Data (Real Predictions)":
            scenario = TEST_SCENARIOS[current_scenario_display]
            st.markdown(f"""
            <div style="background: #fef3c7; padding: 0.8rem; border-radius: 8px; margin-top: 0.5rem;">
                <strong>Weather:</strong> {scenario['weather'].replace('_', ' ').title()}<br>
                <strong>Demand range:</strong> {min(scenario['demand_multipliers']):.0%} - {max(scenario['demand_multipliers']):.0%}
            </div>
            """, unsafe_allow_html=True)
        else:
            st.info("Using live ML predictions")
...
def main():
    ...
    # Check if a test scenario is selected
    selected_scenario = st.session_state.get('selected_scenario', 'Live Data (Real Predictions)')
    
    if selected_scenario != "Live Data (Real Predictions)":
        # ============== USE SCENARIO DATA ==============
        st.info(f"**TEST MODE:** Using scenario '{selected_scenario}'")
        
        # Generate mock data based on scenario
        results_df = generate_scenario_data(selected_scenario, historical_stats)
        
        # Add demand categorization
        ...
        # Add avg_temp for weather metrics
